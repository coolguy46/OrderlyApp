-- Future server-synced task scheduling metadata.
-- The current application intentionally uses the browser-persisted schedule
-- store until this additive migration can be applied to production Supabase.
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint WHERE conname = 'tasks_scheduled_start_requires_date'
  ) THEN
  ...
  END IF;
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint WHERE conname = 'tasks_duration_seconds_valid'
  ) THEN
  ...
  END IF;
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint WHERE conname = 'tasks_schedule_overrides_object'
  ) THEN
