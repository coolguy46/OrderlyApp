    ...
    <Dialog.Root open={open} onOpenChange={next => { if (!next) close(); }}>
      <Dialog.Portal>
        ...
        <Dialog.Content
          ...
          className="fixed inset-x-2 top-1/2 z-[71] flex h-[min(820px,calc(100dvh-1rem))] -translate-y-1/2 flex-col overflow-hidden rounded-2xl border border-border bg-background text-foreground shadow-2xl outline-none sm:inset-x-auto sm:left-1/2 sm:w-[calc(100%-3rem)] sm:max-w-[1120px] sm:-translate-x-1/2"
        >
          <div className="flex shrink-0 items-center justify-between gap-3 border-b border-border bg-card px-4 py-4 sm:px-7 sm:py-5">
            <div className="flex min-w-0 items-center gap-3">
              <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-lg border border-border bg-background text-primary"><BookOpen className="h-5 w-5" aria-hidden="true" /></div>
              <div className="min-w-0">
                <Dialog.Title className="text-sm font-semibold tracking-tight sm:text-base">Your guide to Orderly</Dialog.Title>
          ...
          <div className="flex min-h-0 flex-1 flex-col sm:flex-row">
            {current && <>
              <nav aria-label="Tutorial sections" data-testid="tutorial-section-menu" className="hidden w-56 shrink-0 overflow-y-auto border-r border-border bg-card/50 px-3 py-5 sm:block">
            ...
            <div ref={contentRef} data-testid="tutorial-content" data-tutorial-section={current?.id || 'overview'} className="min-h-0 min-w-0 flex-1 overflow-y-auto overscroll-contain px-4 py-6 sm:p-8">
              {current ? <article>
                ...
                <h2 ref={headingRef} tabIndex={-1} className="font-display text-2xl font-semibold tracking-[-0.035em] outline-none sm:text-3xl">{current.title}</h2>
                ...
                <figure className="my-6 overflow-hidden rounded-xl border border-border bg-card shadow-sm">
                  <div className="flex items-center justify-between gap-2 border-b border-border bg-card px-4 py-3 text-[10px] tracking-wider text-muted-foreground"><span>DEMO · ALEX MORGAN</span><span>SEP 7, 2026</span></div>
                ...
                <div className="mt-5 rounded-r-xl border-l-2 border-primary bg-primary/5 px-4 py-4 text-sm leading-6">{current.takeaway}</div>
                ...
                <h2 ref={headingRef} tabIndex={-1} className="font-display text-3xl font-semibold tracking-[-0.04em] outline-none sm:text-5xl">Make yourself at home.</h2>
                ...
                <div className="mb-8 mt-6 flex flex-wrap items-center gap-3 border-b border-border pb-7">
                ...
                <nav aria-label="Tutorial sections" data-testid="tutorial-section-menu" className="grid grid-cols-1 gap-3 min-[420px]:grid-cols-2 lg:grid-cols-3">
                  {TUTORIAL_SECTIONS.map((item, index) => <button key={item.id} type="button" data-section-id={item.id} onClick={() => selectSection(index)} className="group flex min-h-[96px] items-center gap-3 rounded-xl border border-border bg-card p-4 text-left transition-colors hover:border-primary/40 hover:bg-primary/5 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary">
          ...
          <footer className="shrink-0 border-t border-border bg-card px-3 pb-[max(0.75rem,env(safe-area-inset-bottom))] pt-3 sm:px-7">
...
export function TutorialInvitation() {
  ...
  return <aside aria-label="Welcome guide" className="mb-5 flex flex-wrap items-center gap-3 rounded-xl border border-border bg-card p-3 sm:px-4">
