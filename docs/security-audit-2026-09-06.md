...
The Vercel project serving `www.myorderlyapp.com` is **orderlyappp**, not the other similarly named projects. Production secret-variable entries for the Supabase service role, DeepSeek key and Canvas cron secret were verified present without revealing values. Code release verification will be recorded below. **The audit also found older missing production migrations and owner-security configuration gaps; the whole security rollout is not yet complete.**
...
- Local secret-pattern scan of tracked source, Git history and production client bundles: **0 matches** at the final pre-stage check (94,497 scan units; the scanner mixes files and history-line units). The scanner only prints categories/locations, never the matching secret. Pattern scans can miss unfamiliar credentials; no secret was rotated.
