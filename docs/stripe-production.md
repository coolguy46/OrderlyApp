...
The owner UI explicitly labels complimentary access rather than claiming a paid subscription. Existing Stripe subscription/portal details are preserved when available; nothing creates, cancels, or modifies a subscription. Model safety switches, rate limits, usage accounting and user-data authorization still apply to the owner. Visual preview scenarios and sandbox checkout remain separate and cannot grant real access. No Stripe account setting, customer, database record, secret or environment value was changed.

Owner-access validation: 562 unit/integration tests passed. Tests exercised actual status and AI handlers with synthetic verified identities, spoofed identity/request fields, disabled billing, billing failures, paid/trial accounts, and owner rate-limit enforcement. Browser regression verified owner access and billing copy, account-switch/late-response isolation, and independent visual previews. Targeted lint, isolated TypeScript, clean-source Next production build, and credential-pattern scan passed. The real owner's signed-in production session was not used in these tests.

## September 13, 2026: production enforcement

Production requires a verified active trial or paid subscription for non-owner accounts: `NODE_ENV=production` or `VERCEL_ENV=production` makes the gate mandatory, even when the old `AI_SUBSCRIPTION_REQUIRED` flag is false or missing. The local development opt-out cannot unlock the production site. UI and all three paid provider routes use the same policy; unavailable billing fails closed before provider calls or usage reservations. The later, explicitly approved owner exception is described above; visual preview still grants no access.

When billing is not configured, `/api/billing/status` returns an explicit locked state for non-owner accounts (`subscriptionRequired: true`, `aiAccess: false`, `checkoutEnabled: false`) and the UI says subscriptions are temporarily unavailable. It does not present a dead purchase button or briefly show a lock and then grant free access. Real checkout and database/secret setup are still separate activation gates. Manual tasks/calendar, receipt recovery, and Undo remain available.
