...
async function main() {
  ...
  check(submittedParameters.subscription_data.trial_period_days === undefined, 'new_checkout_must_not_have_trial');
  check(submittedParameters.payment_method_collection === 'always', 'card_not_required');
  check(submittedParameters.subscription_data.trial_settings === undefined, 'unexpected_trial_settings');
  ...
  check(session.payment_method_collection === 'always' && session.metadata.orderly_trial_days === '0', 'checkout_terms_mismatch');
  ...
  check(status.aiAccess === false && status.trialEligible === false, 'open_checkout_granted_access');
  report.customerId = billingAccount.customer_id;
  report.sessionId = session.id;
  report.noTrialImmediatePayment = true;
