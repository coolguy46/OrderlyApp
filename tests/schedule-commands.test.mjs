...
test('a real conflict requires explicit overlap permission', () => {
  const school = {
    id: 'school-1',
    title: 'School day',
  ...
  assert.equal(duringSchool.status, 'clarification');
  assert.equal(duringSchool.actions.length, 0);
  assert.match(duringSchool.summary, /overlaps “School day”.*include “force”/i);

  const forcedDuringSchool = interpretScheduleCommand(
    'Create a task called Study Notes today from 10 AM to 11 AM, force.',
    context({ busy: [school] }),
  );
  assert.equal(forcedDuringSchool.status, 'ready');
  assert.equal(forcedDuringSchool.actions[0].schedule.startAt, '2026-08-27T17:00:00.000Z');
  assert.match(forcedDuringSchool.assumptions.join(' '), /Allowed an overlap with School day.*explicitly said “force”/i);
